            ...
            <div className='overflow-auto custom-scrollbar h-100 w-100 p-2 pe-0'>
                {
                    chatHistory.map((chat) =>
                        <div className={`font-12 p-1 fw-bold card bg-off-white text-black ${JSON.parse(localStorage.getItem('v1:userInfo')).name === chat.sender && 'text-end pe-2'}`}>
                            {JSON.parse(localStorage.getItem('v1:userInfo')).name === chat.sender ? 'YOU' : chat.sender}
